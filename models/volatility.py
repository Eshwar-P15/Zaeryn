...
class VolatilityPredictor:
    ...
    def train(
        ...
        if result is None:
            raise RuntimeError(f"Insufficient data for {asset} — cannot train VolatilityPredictor")
        ...
        logger.info(f"[{asset}] split: train={len(X_train)}, test={len(X_test)} (chronological)")
        ...
        logger.info(
            f"[{asset}] XGB params: {'optimized' if asset in XGB_PARAMS_BY_ASSET else 'default'}"
        )
        ...
        logger.info(f"[{asset}] VolatilityPredictor: MAE={mae:.4f}  RMSE={rmse:.4f}  R²={r2:.4f}")
        return metrics
