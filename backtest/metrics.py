...
def compute_annualized_return(total_return_pct: float, n_candles: int) -> float:
    ...
    annualized = (
        (1 + total_return_decimal) ** (BACKTEST_ANNUALIZATION / n_candles) - 1
    ) * 100.0
...
def compute_metrics(result: BacktestResult) -> dict:
    ...
    if trades:
        ...
        for t in trades:
            try:
                entry = t.entry_time if isinstance(t.entry_time, datetime) else datetime.fromisoformat(str(t.entry_time))
                exit_ = t.exit_time  if isinstance(t.exit_time,  datetime) else datetime.fromisoformat(str(t.exit_time))
