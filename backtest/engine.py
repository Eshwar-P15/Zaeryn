...
class BacktestEngine:
    ...
    def run(
        ...
        logger.info(f"Backtesting [{self.strategy.name}] on {asset} ({days_back}d, {granularity})")
        ...
        if raw.empty or len(raw) < INDICATOR_WARMUP + 10:
            logger.warning(f"[{asset}] Insufficient candles for backtest: {len(raw)} rows")
        ...
        for i in range(INDICATOR_WARMUP, len(df)):
            ...
            if signal.action == "BUY" and position is None:
                position, cash = self._open_position(asset, signal, candle_close, candle_time, cash)
        ...
        if position is not None and len(df) > INDICATOR_WARMUP:
            ...
            trade, cash = self._close_position(position, last_close, "END_OF_DATA", last_time, cash)
            trades.append(trade)
            if equity_curve:
                equity_curve[-1] = cash
