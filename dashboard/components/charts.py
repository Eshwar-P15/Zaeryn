...
def _base_layout(height: int, title: str = "") -> dict:
    ...
    if title:
        layout["title"] = {
            "text":     title,
            "font":     {"family": "Bebas Neue", "size": 20,
...
def candlestick_chart(df: pd.DataFrame, asset: str,
                      show_sma: bool = True, height: int = 500) -> go.Figure:
    fig = make_subplots(
        rows=2, cols=1,
        shared_xaxes=True,
...
def equity_curves(curves: dict, timestamps: list,
                  height: int = 500) -> go.Figure:
    fig = go.Figure()

    STYLE_MAP = {
        "ZAERYN ML Composite": {"color": COLORS["accent_glow"],  "width": 3,   "dash": "solid", "opacity": 1.0, "fill": True},
        "Buy & Hold":          {"color": "#555555",              "width": 1.5, "dash": "dot",   "opacity": 0.7, "fill": False},
        "MACD Cross":          {"color": "#333333",              "width": 1.5, "dash": "solid", "opacity": 0.8, "fill": False},
        "RSI Mean Reversion":  {"color": "#2A2A2A",              "width": 1.5, "dash": "solid", "opacity": 0.8, "fill": False},
        "Bollinger Band":      {"color": "#252525",              "width": 1.5, "dash": "solid", "opacity": 0.8, "fill": False},
    }

    for name, values in curves.items():
        if not values:
            continue
        style    = STYLE_MAP.get(name, {"color": "#333", "width": 1,
                                        "dash": "solid", "opacity": 0.6,
                                        "fill": False})
    ...
    fig.add_hline(y=0, line_dash="dot",
                  line_color=COLORS["border_default"], opacity=0.5)
    return fig


def sharpe_bar(strategies: list, sharpes: list, height: int = 380) -> go.Figure:
    colors = [
        COLORS["accent_primary"] if "ZAERYN" in s
        else "#222222" if v >= 0
        else "#1A0000"
        for s, v in zip(strategies, sharpes)
    ]
    ...
    fig.add_vline(x=0, line_dash="dot",
                  line_color=COLORS["border_default"], opacity=0.6)
    fig.update_layout(
...
def sentiment_bars(assets: list, scores: list, height: int = 260) -> go.Figure:
    colors = [
        COLORS["accent_primary"] if s < -0.1
        else COLORS["text_primary"] if s > 0.1
        else "#444444"
        for s in scores
    ]
    ...
    fig = go.Figure(go.Bar(
        ...
        textposition="outside",
        textfont={"family": "Space Mono", "size": 10,
                  "color": COLORS["text_secondary"]},
    ))
...
def feature_importance(features: list, values: list, height: int = 440) -> go.Figure:
    pairs = sorted(zip(values, features), reverse=True)[:15]
    vals, feats = zip(*pairs) if pairs else ([], [])

    colors = [
        COLORS["accent_primary"] if i == 0
        else COLORS["accent_dim"] if i < 3
        else "#222222"
    ...
    fig = go.Figure(go.Bar(
        ...
        textposition="outside",
        textfont={"family": "Space Mono", "size": 9,
...
def model_performance_bars(assets: list, r2s: list,
                            aucs: list, height: int = 360) -> go.Figure:
    fig = go.Figure()
    ...
    fig.add_hline(y=0.5, line_dash="dot", line_color="#333333", opacity=0.6,
                  annotation_text="AUC baseline",
                  annotation_font={"size": 9, "color": "#444"})
    return fig


def scatter_risk_return(names: list, returns: list,
                        sharpes: list, trades: list,
                        height: int = 460) -> go.Figure:
    fig = go.Figure()

    for name, ret, sharpe, tc in zip(names, returns, sharpes, trades):
        is_z   = "ZAERYN" in name
        ...
        fig.add_trace(go.Scatter(
            ...
            marker={
                ...
                "line":    {"color": "rgba(255,61,0,0.4)" if is_z else "#000",
