...
from dashboard.components.theme import (
    ...
    chart_container_close, COLORS,
)
from dashboard.components.charts import feature_importance, model_performance_bars
from dashboard.data_loader import model_health_all, feature_importances
...
def render():
        ...
        if os.path.exists(vp) and os.path.exists(tp):
            try:
                ...
                rows.append({
                    ...
                    "TREND AUC":  round(td.get("auc",  0), 4) if td.get("auc")  is not None else "—",
    ...
    if active_rows:
        ...
        fig = model_performance_bars(
            ...
            [r["TREND AUC"] if isinstance(r.get("TREND AUC"), float) else 0.0
                        ...
                        <div style="text-align:right;">
                            <div style="font-family:'Space Mono',monospace; font-size:0.7rem; color:#555;">
                                ...
                                {f"{health.get(asset, {}).get('volatility_prediction', 0):.3f}"
                                 if health.get(asset, {}).get("models_exist") else "—"}</span>
                            </div>
                            <div style="font-family:'Space Mono',monospace; font-size:0.7rem;
                                        color:{'#F5F5F5' if (health.get(asset,{}).get('trend') or {}).get('direction') == 'UP' else '#FF2D00' if (health.get(asset,{}).get('trend') or {}).get('direction') == 'DOWN' else '#444'};">
                                {(health.get(asset, {}).get("trend") or {}).get("direction", "—")}
                                ({(health.get(asset, {}).get("trend") or {}).get("confidence", 0):.2f})
                            </div>
                        </div>
                    </div>'''
