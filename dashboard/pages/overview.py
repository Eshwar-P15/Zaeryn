...
from dashboard.data_loader import candles, db_stats, equity_curves_for, strategy_comparison
...
def render():
    ...
    # -- Hero stats -----------------------------------------------------------
    zaeryn_df = (
        df[df["Strategy"].str.contains("ZAERYN", na=False)] if not df.empty else pd.DataFrame()
    )
    ...
    c4.markdown(
        stat_block(str(len(FEATURE_COLUMNS)), "Features", accent=False), unsafe_allow_html=True
    )
    c5.markdown(
        stat_block(f"{_total_candles:,}", "Candles Processed", accent=False), unsafe_allow_html=True
    )
    ...
    st.markdown(
        chart_container("", "90 days · $10,000 starting capital · 4 strategies + buy & hold"),
        unsafe_allow_html=True,
    )
    ...
    with col1:
        ...
        if not df.empty:
            ...
            btc_c = candles("BTC-USD", 90)
            if not btc_c.empty:
                bah_ret = (float(btc_c["close"].iloc[-1]) / float(btc_c["close"].iloc[0]) - 1) * 100
                btc = pd.concat(
                    [
                        btc,
                        pd.DataFrame(
                ...
                [
                    ...
                    for icon, title, desc in [
                        ...
                        (
                            "▲",
                            "ML MODELS",
                            f"xgboost + rf<br>{_best_r2}<br>{len(FEATURE_COLUMNS)} features",
                        ),
