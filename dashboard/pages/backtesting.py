...
from dashboard.data_loader import strategy_comparison, equity_curves_for, candles
...
def render():
    ...
    z_sharp  = z_active["Sharpe"].mean() if not z_active.empty else (zaeryn["Sharpe"].mean() if not zaeryn.empty else 0)
    ...
    c1.markdown(stat_block(f"{z_sharp:.2f}",   "ZAERYN Avg Sharpe", accent=True), unsafe_allow_html=True)
