...
from dashboard.data_loader import sentiment_all, fear_greed
...
def render():
    ...
    fg  = fear_greed()
    fgv = fg.get("value", 50)
    fgn = fg.get("normalized", 0.0)
    fgl = fg.get("label", "Neutral")
    ...
    c4.markdown(stat_block(str(bearish),         "Bearish Assets",  accent=bearish > 5), unsafe_allow_html=True)
