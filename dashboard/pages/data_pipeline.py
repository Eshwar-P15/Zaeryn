...
from dashboard.data_loader import candles, db_stats
...
def render():
    ...
    with col1:
        asset_sel = st.selectbox(
            "ASSET",
            [a for a in ALL_ASSETS if stats.get(a, {}).get("1h", {}).get("count", 0) > 100],
        )
    with col2:
    ...
    if not df.empty:
        ...
        c = st.columns(5)
        c[0].markdown(
            stat_block(f"${float(latest['close']):,.2f}", "Current Price"), unsafe_allow_html=True
        )
        ...
        c[3].markdown(
            stat_block(f"${float(df['high'].max()):,.2f}", "Period High"), unsafe_allow_html=True
        )
        c[4].markdown(
            stat_block(f"${float(df['low'].min()):,.2f}", "Period Low"), unsafe_allow_html=True
        )
