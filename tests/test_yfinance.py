...
def _make_yf_df(
    n: int = 50, with_volume: bool = True, use_date_index: bool = False
) -> pd.DataFrame:
    """Mock DataFrame matching yfinance.download() output."""
    base = datetime(2024, 1, 1, tzinfo=UTC)
    idx_name = "Date" if use_date_index else "Datetime"
    idx = pd.DatetimeIndex([base + timedelta(hours=i) for i in range(n)], name=idx_name)
    price = 150.0
    rows = []
    for _ in range(n):
...
class TestFetchYfinanceOhlcv:
    ...
    def test_forex_zero_volume_accepted(self):
        ...
        with patch(
            "data.yfinance_fetcher.yf.download", return_value=_make_yf_df(20, with_volume=False)
        ):
    ...
    def test_multilevel_columns_flattened(self):
        ...
        raw = _make_yf_df(20)
        raw.columns = pd.MultiIndex.from_tuples([(col, "AAPL") for col in raw.columns])
    ...
    def test_1h_days_back_capped_at_730(self):
        ...
        start = datetime.strptime(captured["start"], "%Y-%m-%d").replace(tzinfo=UTC)
        expected = datetime.now(UTC) - timedelta(days=729)
    ...
    def test_date_index_also_handled(self):
        ...
        with patch(
            "data.yfinance_fetcher.yf.download", return_value=_make_yf_df(10, use_date_index=True)
        ):
            result = fetch_yfinance_ohlcv("AAPL")
        ...
        df = pd.DataFrame(
            {
                "timestamp": pd.to_datetime(
                    [base + timedelta(hours=i) for i in range(n)], utc=True
                ),
                "open": [100.0] * n,
                "high": [101.0] * n,
                "low": [99.0] * n,
