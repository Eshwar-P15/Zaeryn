...
from sentiment.cache import cache_sentiment, clear_cache, load_cached_sentiment
...
from sentiment.onchain import compute_whale_concentration, fetch_onchain_sentiment
from sentiment.scorer import compute_sentiment_score, score_label
...
def test_dex_sentiment_buy_dominated():
    ...
    with (
        patch("sentiment.dex_sentiment._fetch_pair_stats", return_value=mock_pair),
...
# --- onchain routing tests (regression for docs/repo_audit.md §8) ---


def test_onchain_runs_for_birdeye_routed_assets(monkeypatch):
    """JUP/BONK/WIF/PYTH/RAY route via 'birdeye' but are still Solana tokens.
    The on-chain analyzer must NOT short-circuit them. Regression test for
    the dead branch identified in docs/repo_audit.md section 8."""
    monkeypatch.setenv("HELIUS_API_KEY", "fake-test-key")
    fake_holders = [
        {"uiAmount": 100.0},
        {"uiAmount": 80.0},
        {"uiAmount": 60.0},
    ...
    fake_supply = 10_000.0  # → whale_pct = 3% → healthy distribution branch

    for asset in ("JUP", "BONK", "WIF", "PYTH", "RAY"):
        with (
            patch("sentiment.onchain.fetch_token_holders", return_value=fake_holders),
            patch("sentiment.onchain.fetch_token_supply", return_value=fake_supply),
        ):
            result = fetch_onchain_sentiment(asset)

        # NOT the early-exit shape
        assert "note" not in result, (
            f"{asset} short-circuited with note={result.get('note')!r} — "
            "on-chain dead branch regression"
        )
        assert result["confidence"] > 0, f"{asset} returned zero confidence"
        assert "whale_concentration" in result, f"{asset} missing whale_concentration"
        assert result["holder_count"] == len(fake_holders)
        assert result["asset"] == asset


def test_onchain_skips_non_solana_assets():
    """Stocks, forex, and Coinbase crypto should still short-circuit before
    any Helius call. Confirms the routing fix didn't over-correct."""
    for asset in ("BTC-USD", "AAPL", "EUR-USD"):
        result = fetch_onchain_sentiment(asset)
        assert result["confidence"] == 0.0, f"{asset} should short-circuit"
        assert result["score"] == 0.0
        assert "note" in result, f"{asset} took the Helius path (it should not)"
...
def test_cache_expiry(tmp_path, monkeypatch):
    monkeypatch.setattr("sentiment.cache.CACHE_DIR", str(tmp_path))
    fake = {"asset": "JUP", "score": 0.1, "timestamp": datetime.now(UTC)}
...
def test_cache_clear(tmp_path, monkeypatch):
    monkeypatch.setattr("sentiment.cache.CACHE_DIR", str(tmp_path))
    fake = {"asset": "BTC-USD", "score": 0.2, "timestamp": datetime.now(UTC)}
...
def test_compute_sentiment_score_structure():
    with (
        patch("sentiment.scorer.fetch_onchain_sentiment", return_value=_MOCK_RESULT),
...
def test_compute_sentiment_score_all_zeros():
    ...
    with (
        patch("sentiment.scorer.fetch_onchain_sentiment", return_value=null),
...
def test_full_score_live():
    ...
    for src, detail in result["components"].items():
        print(
            f"  {src:<15} score={detail['score']:+.3f}  conf={detail['confidence']:.2f}  weight={detail['weight_used']:.3f}"
        )
