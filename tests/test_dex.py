...
from config.settings import ALL_ASSETS, ASSET_SOURCE, SOLANA_TOKENS
...
def test_select_best_pair_falls_back_below_threshold():
    ...
    assert best.get("_below_threshold")
...
def test_fetch_candles_routes_coinbase():
    with (
        patch("data.historical._fetch_from_coinbase") as mock_cb,
...
def test_fetch_dex_candles_live():
    ...
    print(
        df[["timestamp", "open", "high", "low", "close", "volume"]].tail(3).to_string(index=False)
    )
