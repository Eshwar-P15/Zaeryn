...
from models.features import compute_targets, compute_technical_indicators
...
def test_williams_r_range():
    ...
    if "williams_r_14" in result.columns:
        ...
        assert (valid >= -100).all() and (valid <= 0).all(), "Williams %R out of [-100, 0] range"
...
def test_bb_position_range():
    ...
    if "bb_position" in result.columns:
        ...
        assert (valid >= -0.5).all() and (valid <= 1.5).all(), "bb_position wildly out of range"
...
def test_target_direction_values():
    ...
    assert set(non_nan.unique()).issubset(
        {0.0, 1.0}
