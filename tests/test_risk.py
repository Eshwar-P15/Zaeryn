...
from risk.scorer import (
    risk_label, recommendation, _redistribute_weights,
    _compute_sentiment_component, _compute_momentum_component,
    compute_risk_score,
)
from risk.position_sizer import (
    kelly_fraction, risk_adjusted_size,
    stop_loss_price, take_profit_price,
)
from risk.alerts import Alert, check_alerts
from config.settings import (
    RECOMMEND_TRADE, RECOMMEND_HOLD, RECOMMEND_REDUCE,
    MAX_POSITION_PCT, ALERT_RISK_EXTREME, KELLY_FRACTION,
    RISK_REWARD_RATIO,
)
...
def make_candles(n: int = 100, seed: int = 42) -> pd.DataFrame:
    ...
    timestamps = pd.date_range(end=datetime.now(timezone.utc), periods=n, freq="1h")
    df = pd.DataFrame({
...
def test_redistribute_all_missing():
    components = {k: (None, 0.0) for k in
                  ["volatility", "trend_uncertainty", "sentiment",
                   "price_momentum", "market_regime"]}
...
def test_sentiment_component_cache_miss_api_down():
    with patch("risk.scorer.load_cached_sentiment", return_value=None), \
...
def test_kelly_never_exceeds_cap():
    ...
    for up_prob in [0.6, 0.7, 0.8, 0.9, 0.99]:
        ...
        assert k <= max_allowed + 1e-9, \
            f"Kelly {k} exceeds max {max_allowed} at up_prob={up_prob}"
...
def test_risk_score_always_in_range():
    with patch("risk.scorer.load_candles",                   return_value=make_candles(100)), \
...
def test_full_position_plan_live():
    ...
    print(f"  Position:    {plan['position']['position_pct']*100:.2f}% (${plan['position']['position_usd']:.2f})")
