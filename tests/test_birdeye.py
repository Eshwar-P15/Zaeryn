...
def test_fetch_chunk_401_raises():
    ...
    mock = MagicMock(); mock.status_code = 401
    with patch("data.birdeye_fetcher.requests.get", return_value=mock):
...
def test_fetch_chunk_200_returns_items():
    ...
    with patch("data.birdeye_fetcher.requests.get", return_value=mock):
        with patch("data.birdeye_fetcher.time.sleep"):
...
def test_stops_at_launch_date():
    ...
    with patch.object(birdeye_fetcher, "_fetch_chunk", side_effect=mock_chunk):
        with patch("data.birdeye_fetcher.time.sleep"):
...
def test_returns_none_when_all_chunks_fail():
    from data import birdeye_fetcher
    birdeye_fetcher.BIRDEYE_API_KEY = "key"
    with patch.object(birdeye_fetcher, "_fetch_chunk", return_value=None):
        with patch("data.birdeye_fetcher.time.sleep"):
...
def test_bonk_live():
    ...
    from config.settings import SOLANA_TOKENS, BIRDEYE_API_KEY
